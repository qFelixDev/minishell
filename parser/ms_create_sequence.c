	...
	while (object_index < object_count)
	{
		if (sequence -> objects[object_index] != NULL)
		{
			if (sequence -> is_sequence[object_index / 8]
				...
				ms_free_tokens(sequence -> objects[object_index]);
		}
		object_index++;
	}
...
{
	int						bracket_level;
	t_ms_sequence *const	sequence = ft_calloc(sizeof(t_ms_sequence));

	if (sequence == NULL)
		return (NULL);
	sequence -> object_count = 0;
	bracket_level = 0;
	token = token -> next;
	...
	sequence -> objects = ft_calloc(sizeof(void*) * sequence -> object_count);
	sequence -> is_sequence
		= ft_calloc(sizeof(uint8_t) * (sequence -> object_count / 8 + 1));
...
{
	size_t					object_index;
	t_ms_sequence *const	sequence = allocate_sequence(token -> next);
